        ...
        <div style={{ position: 'relative' }}>
          <button
            ...
            style={{
              ...
              border: `1px solid ${selected ? 'rgba(245,158,11,0.4)' : 'var(--color-border)'}`,
          ...
          >
            ...
            {currentName && <Check size={13} color="var(--color-gold)" />}
          </button>
          ...
          {open && (
            <div style={{
              ...
              background: '#0d1a3a', border: '1px solid rgba(245,158,11,0.3)',
              boxShadow: '0 16px 40px rgba(0,0,0,0.6)',
            }}>
              <div style={{ padding: '10px 12px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                <div style={{ position: 'relative' }}>
                  <Search size={12} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--color-text-subtle)' }} />
                  <input
                    autoFocus
                    placeholder="Search…"
                    value={search}
              ...
              <div style={{ maxHeight: 220, overflowY: 'auto' }}>
                {filtered.length === 0 ? (
                  <div style={{ padding: '16px 14px', fontSize: '0.8rem', color: 'var(--color-text-subtle)', textAlign: 'center' }}>
                    No players found
                  </div>
                ) : (
                  filtered.map((p) => (
                    <button
                      ...
                      style={{
                        ...
                        padding: '9px 14px', background: selected === p.id ? 'rgba(245,158,11,0.1)' : 'transparent',
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: '0.83rem', fontWeight: 600, color: selected === p.id ? '#f59e0b' : '#f1f5f9', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {p.name}
                        </div>
                        <div style={{ fontSize: '0.68rem', color: 'var(--color-text-subtle)' }}>
                          {p.team?.name} · {p.position || '—'}
                        </div>
                      </div>
                      {selected === p.id && <Check size={13} color="var(--color-gold)" />}
                    </button>
                  ))
                )}
              </div>
...
export default function PlayerPicksSection({
  ...
  useEffect(() => {
    // Lazy load players when the section is first mounted
    setLoading(true)
